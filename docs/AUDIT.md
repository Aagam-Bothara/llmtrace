...
| Experiments | Work-identical replay (`ignore_eos`, token-id prompts, seeds), manifests with scheduled vs actual arrivals and source provenance (fingerprint, commit, dirty patch), `decide` with eligibility rules, `--min-repeats`, run-to-run range and a within-run request bootstrap, goodput under SLOs | Most GPU evidence has two repeats per configuration; the request bootstrap understates run-to-run uncertainty (requests share steps) and is labelled as such; KV pressure validated on opt-125m only |
