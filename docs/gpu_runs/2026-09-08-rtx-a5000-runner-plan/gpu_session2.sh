#!/usr/bin/env bash
# GPU session 2, stage 2: engine runs (the first stage's log redirects targeted directories that did not exist).
set -u
...
mkdir -p "$OUT/runner" "$OUT/driver"
S="$OUT/summary2.txt"; : > "$S"
log() { echo "$*" | tee -a "$S"; }
cd "$REPO"
...
for i in 0 1; do
  for cfg in baseline capped; do
    ...
    log "runner $cfg $i exit=$? :: $(grep -E '^\[' "$d.log" | tail -1 | cut -c1-220)"
    grep -E "scheduler visible|PROBLEM|telemetry:|FAILED" "$d.log" | tee -a "$S"
  done
done

# B. experiment driver on the same workload defaults, one each
...
cat "$OUT/doctor_run.txt" | tee -a "$S"
...
grep -E "^\[" "$OUT/findings_runner_baseline_0.txt" | cut -c1-220 | tee -a "$S"

log "=== analyze.py on runner and driver runs"
...
for f in analysis_runner_0 analysis_runner_1 analysis_driver_0; do
  log "--- $f"; grep -E "^verdict|short_ttft_ms_p95|short_itl_ms_max|long_ttft_ms_p50|long_chunk|gpu_split" "$OUT/$f.txt" | cut -c1-200 | tee -a "$S"
done

log "=== decide: runner repeats + driver repeat"
...
grep -E "^\[|PROBLEM|warning|compare with|FAILED" "$OUT/planned.log" | cut -c1-220 | tee -a "$S"
CFGS=$(python - <<'EOF'
